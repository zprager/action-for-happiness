...
The email system is deliberately a test setup so that would break. The email cron is just an endpoint that sends to all users on demand. In production, I would move this to a proper scheduler that's sensitive to each user's timezone.  I would normally separate the DB from the Docker container.
