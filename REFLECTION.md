...
The email system is deliberately a test setup so that would break and it is insecure. The email cron is just an endpoint that sends to all users on demand. In production, I would move this to a proper scheduler that's sensitive to each user's timezone.  I would normally separate the DB from the Docker container.
...
Fixes:  I had to fix design issues.  Multiple redundant pages.  State issues around the login/logout. 
